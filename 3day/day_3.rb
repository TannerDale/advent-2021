...
# the epsilon rate is 01001, or 9 in decimal. Multiplying the gamma rate (22)
...
def part1(input)
  outcomes = Hash.new { |h, k| h[k] = { '0' => 0, '1' => 1 } }

  input.each do |bits|
    bits.chars.each_with_index do |bit, i|
      outcomes[i][bit] += 1
    end
  end

  gamma = outcomes.map do |_index, bits|
    bits.max_by { |_k, v| v }.first
  end

  epsilon = outcomes.map do |_index, bits|
    bits.min_by { |_k, v| v }.first
  end
...
# that bit in that position. If 0 and 1 are equally common, keep values with a 1 in the position being considered.
...
# 7 numbers with a 1 in the first position:
...
#  only the 5 numbers with a 0 in the first position:
...
def part2(data, num, type)
  count = { '0' => { amount: 0, index: [] }, '1' => { amount: 0, index: [] } }

  data.each_with_index do |bits, i|
    count[bits[num]][:amount] += 1
    count[bits[num]][:index] << i
  end
  want = type == :max ? max_stuff(count) : min_stuff(count)
  want = want[:index]
  data = data.values_at(*want)

  return data.first if data.size == 1

  part2(data, num + 1, type)
end

def max_stuff(data)
  return data['1'] if same?(data)

  data.max_by { |_k, v| v[:amount] }.last
end

def min_stuff(data)
  return data['0'] if same?(data)

  data.min_by { |_k, v| v[:amount] }.last
end
...
prac_result = part2(prac_input, 0, :max).to_i(2) * part2(prac_input, 0, :min).to_i(2)
answer = part2(input, 0, :max).to_i(2) * part2(input, 0, :min).to_i(2)

puts '--- Part 2 ---'
puts "Practice: #{prac_result}"
puts "Answer: #{answer}"
